...
test('step 6 waits for registration success and completes from background', async () => {
  ...
  assert.deepStrictEqual(events.waits, [4000]);
  assert.deepStrictEqual(events.completedSteps, ['wait-registration-success']);
  assert.ok(events.logs.some(({ message }) => /等待 4 秒/.test(message)));
});
...
test('local cpa json no-RT export runs as step 7 after Plus checkout completes', async () => {
  const source = fs.readFileSync('background/steps/wait-registration-success.js', 'utf8');
  const globalScope = {};
  const api = new Function('self', `${source}; return self.MultiPageBackgroundStep6;`)(globalScope);
  ...
  assert.equal(events.tabRemove, 17);
  assert.equal(events.ensureCalls.length, 1);
  assert.equal(events.ensureCalls[0].sourceId, 'plus-checkout');
  assert.deepStrictEqual(events.ensureCalls[0].options.inject, ['content/utils.js', 'content/operation-delay.js', 'content/plus-checkout.js']);
  assert.equal(events.sendCalls.length, 1);
...
test('step 7 falls back to the current hotmail pool email when runtime email is empty', async () => {
  const source = fs.readFileSync('background/steps/oauth-login.js', 'utf8');
  const globalScope = {};
  const api = new Function('self', `${source}; return self.MultiPageBackgroundStep7;`)(globalScope);
  ...
  const executor = api.createStep7Executor({
    ...
    getState: async () => ({
      ...
      hotmailAccounts: [
        { id: 'hot-1', email: 'pool-user@hotmail.com' },
      ],
    }),
    isStep6RecoverableResult: (result) => result?.step6Outcome === 'recoverable',
    isStep6SuccessResult: (result) => result?.step6Outcome === 'success',
  ...
  await executor.executeStep7({ password: 'secret' });

  assert.equal(events.payloads.length, 1);
  assert.equal(events.payloads[0].email, 'pool-user@hotmail.com');
  assert.equal(events.payloads[0].accountIdentifier, 'pool-user@hotmail.com');
  assert.equal(events.payloads[0].loginIdentifierType, 'email');
});

test('step 7 prefers the latest runtime email over a stale empty state payload', async () => {
  const source = fs.readFileSync('background/steps/oauth-login.js', 'utf8');
  const globalScope = {};
  const api = new Function('self', `${source}; return self.MultiPageBackgroundStep7;`)(globalScope);
  ...
  await executor.executeStep7({
    email: '',
    password: 'secret',
    oauthUrl: 'https://oauth.example/stale',
  });

  assert.equal(events.payloads.length, 1);
  assert.equal(events.payloads[0].email, 'fresh@example.com');
  assert.equal(events.payloads[0].accountIdentifier, 'fresh@example.com');
});
