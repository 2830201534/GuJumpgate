...
test('panel bridge can request local cpa json oauth url via local oauth module', async () => {
  const source = fs.readFileSync('background/panel-bridge.js', 'utf8');
  const api = new Function('self', `${source}; return self.MultiPageBackgroundPanelBridge;`)({});
  const bridge = api.createPanelBridge({
    addLog: async () => {},
    createLocalCliProxyApi: () => ({
      createAuthorizationRequest: async () => ({
        oauthUrl: 'https://auth.openai.com/authorize?state=local-oauth-state',
        oauthState: 'local-oauth-state',
        pkceCodes: {
          codeVerifier: 'verifier-local',
          codeChallenge: 'challenge-local',
        },
      }),
    }),
    getPanelMode: () => 'local-cpa-json',
  ...
  assert.deepStrictEqual(result, {
    oauthUrl: 'https://auth.openai.com/authorize?state=local-oauth-state',
    localCpaJsonOAuthState: 'local-oauth-state',
