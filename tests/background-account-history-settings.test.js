...
test('background account history settings are normalized independently from hotmail service mode', () => {
  const bundle = [
    ...
    extractFunction('normalizeLocalCpaJsonPluginDir'),
    extractFunction('normalizeLocalCpaJsonRelativeAuthDir'),
...
const CLOUD_MAIL_PROVIDER = 'cloud-mail';
...
return {
  ...
  assert.equal(
    api.normalizePersistentSettingValue('localCpaJsonPluginDir', ' C:\\plugin '),
    'C:\\plugin'
  );
  assert.equal(
